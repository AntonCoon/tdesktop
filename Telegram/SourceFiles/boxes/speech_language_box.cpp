...
void SpeechLanguageBox::openAndCloseBox(const QString language) {
    FullMsgId itemId = _itemId;
    DocumentData* data = _data;

    closeBox();

    SpeechToText* instance = SpeechToText::create_instance();
        QObject::connect(instance, &SpeechToText::recognized, instance, [data](QString message) {
            Ui::show(Box<SpeechBox>(message));
            data->finishSpeechRecognizing(message);
    });

    data->startSpeechRecognizing();
    instance->execute(itemId, data, language);
}

void SpeechLanguageBox::prepare() {
    addButton(langFactory(lng_box_ok), [=] { openAndCloseBox(Lang::LanguageIdOrDefault(Lang::Current().id())); });
    ...
    select->setCancelledCallback([=] {
        QMessageBox::information(nullptr, "Speech to text", "Cancel");
        select->clearQuery();
    });

    inner->activations(
    ) | rpl::start_with_next([=](const Language &language) {
          openAndCloseBox(language.id);
    }, inner->lifetime());
...
void SpeechLanguageBox::keyPressEvent(QKeyEvent *e) {
...
int SpeechLanguageBox::rowsInPage() const {
    return std::max(height() / Rows::DefaultRowHeight(), 1);
}

void SpeechLanguageBox::setInnerFocus() {
    _setInnerFocus();
}

not_null<Ui::MultiSelect*> SpeechLanguageBox::createMultiSelect() {
...
base::binary_guard SpeechLanguageBox::Show(FullMsgId id, DocumentData *data) {
    ...
    if (manager->languageList().empty()) {
        ...
        **alive = manager->languageListChanged().add_subscription([=] {
            ...
            *alive = nullptr;
            if (show) {
                Ui::show(Box<SpeechLanguageBox>(id, data));
            }
        });
    } else {
        Ui::show(Box<SpeechLanguageBox>(id, data));
